...
void ForwardSub()
{
	...
	int device_part = 100 - percentage;
	int chk = 8000000 * device_part/100;
	/*
	double total_size = 1000000 - chk * 100;
	size_t dummy_size = (int)sqrt(total_size);
	printf("dummy_size: %d\n", dummy_size);
	dummy_size = dummy_size * 100;
	*/
	size_t max_size = 100000;
	size_t dummy_size = max_size * max_size * sizeof(float) - chk;
	cudaMalloc((void **) &dummy_array, dummy_size);
	cudaMemset(dummy_array, 1, dummy_size);
	...
	//cudaMemGetInfo(&free_mem, &total_mem); // insert for get memory information

	//cudaFree(m_cuda);
	//cudaFree(a_cuda);
	//cudaFree(b_cuda);
	//cudaMemGetInfo(&free_mem1, &total_mem2);
}
